...
// Helper: build a graph from an adjacency list keyed 1..=n. Returns the node at val=1.
...
// Collect (val -> sorted neighbor vals) via BFS.
...
// Check no node in `clone` is identical (same Rc) to any node in `original`.
fn is_deep_clone(
...
fn test_canonical_four_node_example() {
    // LeetCode example: 1-2, 1-4, 2-3, 3-4
    let g = build_graph(&[
        (1, vec![2, 4]),
        (2, vec![1, 3]),
        (3, vec![2, 4]),
...
fn test_self_loop() {
    ...
    // self-loop should still point to the clone (same Rc as `c`).
